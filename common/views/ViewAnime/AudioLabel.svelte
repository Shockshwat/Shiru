<script>
    ...
    function setLabel() {
        ...
        if (media?.idMal && dubLists?.dubbed) {
            const episodeOrMedia = !episode || (matchPhrase(data.parseObject?.language, 'English', 3) || matchPhrase(data.parseObject?.file_name, ['Multi Audio', 'Dub', 'Dual', 'Dual Audio', 'English Audio', 'English Dub'], 3))
